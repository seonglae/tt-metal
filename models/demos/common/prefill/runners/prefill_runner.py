...
def _serve_request(runtime, kv_caches, mesh_device, hf_config, rank: int, num_ranks: int, is_first_rank: bool) -> None:
    ...
    # PREFILL_ENABLE_MIGRATION picks who consumes the merged KV chunk table, not whether one exists:
    # 1 hands it to a live migration worker and blocks on its ready, 0 leaves it on disk for an
    # offline reader. PREFILL_MIGRATION_EXPORT_TO_FILE is the independent device-map transport.
    ...
    if num_ranks > 1 and is_per_host_storage(table_path):
        ...
        # Nobody named a path, so nobody is waiting on one; a reader that is would have set it,
        # and prefill_producer refuses to start against a per-host path of its own accord.
        logger.warning(
