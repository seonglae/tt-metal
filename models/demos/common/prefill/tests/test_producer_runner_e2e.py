...
SCENARIOS = {
    ...
    #    The runner builds that single merged table and the producer reads
...
def _scenario_env(sc: dict, **extra) -> dict:
    ...
    `extra` (the layer-ack transport for the runner; the schedule + CHECK_PCC for the producer). Both roles must
...
def _running_runner(tag: str, sc: dict, **extra):
    """Spin up ONE runner (request mode, KV table left on disk) for a scenario and tear it down. Yields the
