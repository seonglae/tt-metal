...
class _TracyCounterView:
    ...
    _BANK_REF = {
        ...
        "TDMA_PACK": ("PACKER_BUSY", "PACKER_DEST_READ_AVAILABLE", "MATH_NOT_SCOREBOARD_STALLED"),
...
def compute_metrics_per_op(perf_counter_df, device_arch=""):
    ...
    For each op (run_host_id, trace_id_count) and each reader, build a _TracyCounterView and call the
    shared perf_metrics_common.compute_metrics (plus compute_l1_client_metrics for Quasar's dynamically
    named l1_client counters); then reduce across readers to min/median/max/avg per metric key
    (None/NaN excluded). Returns {op_key: {metric_key: {min, median, max, avg}}}.

    A reader is (core_x, core_y, risc_type): tt-1xx has one BRISC reader per core, while Quasar's DM0
    reads all four NEOs of a core and extract_perf_counters labels each NEO's records QUASAR_NEO{n}, so
    grouping by risc_type keeps every NEO its own sample instead of collapsing a core onto one.
    """
    import math
